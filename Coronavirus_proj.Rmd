...
* can we confirm recovery rate?

* Can we confirm the number of deaths per recovery is 7.47 (US)? 

* What is unemployment rate doing during all of this?
...
US_Cdat_geo<-US_Cdat %>% select(`Province/State`,Lat,Long) %>% left_join(.,US_Cdat_geo)


leaflet(US_Cdat_geo) %>% 
  setView(lng=-96.7265	,lat=38.5266	,zoom = 4.4) %>% 
  addTiles() %>% 
  addCircleMarkers(lng = US_Cdat$Long,lat = US_Cdat$Lat) 
...
#### Data Analysis
```{r}
#linear trend in deaths vs recovery
Ita_stats %>% plot_ly(x=~Recovered,y=~Deaths)
```

```{r}
Ita_lm_fit<-(lm(Deaths~Recovered,Ita_stats) )
```

```{r}
par(mfrow=c(2,2))
plot(Ita_lm_fit)
#indicates viotaion of normality and homoscedasticity
#could try a transformation but data seems to violate 3/4 assumptions
```
