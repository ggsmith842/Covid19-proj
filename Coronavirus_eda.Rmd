...
api_data <- api_data %>% mutate(total = confirmed - deaths - recovered) %>% group_by(province) %>% summarise(total=sum(total))
```

#### Current Counts (US)

```{r,message=FALSE,warning=FALSE}
api_data %>% plot_ly(x=~total,y=~province,color=~province)
...
US_Cdat_geo<-Global_Confirmed %>% filter(`Country/Region`=="US") %>% group_by(`Province/State`) %>%  summarise(Count=sum(as.numeric(Value))) 

US_Cdat_geo<-Global_Confirmed %>% filter(`Country/Region`=="US") %>% select(`Province/State`,Lat,Long) %>%  inner_join(.,US_Cdat_geo) %>% mutate(Lat=as.numeric((Lat)),Long=as.numeric(Long),Count=as.numeric(Count))
